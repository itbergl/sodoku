...
def displayNum(num, r, c, colour):
    ...
    screen.blit(static_num_dsp,
                ((c + 0.25) * cell_size, (r + 0.25) * cell_size))
        ...
        for c in range(9):
            for v in ghost[r][c]:
                if v != 0:
                    ...
                    ghost_num_dsp = num_font.render(str(v), True,
                                                    (100, 100, 100))
...
def drawBoard():
    ...
    for r in range(9):
        ...
        if r % 3 == 0:
            thickness = 2
        pygame.draw.line(screen, BLACK, (r * cell_size, 0),
                         (r * cell_size, 9 * cell_size), thickness)
    for c in range(10):
        thickness = 1
        if c % 3 == 0:
            thickness = 2
        pygame.draw.line(screen, BLACK, (0, c * cell_size),
                         (9 * cell_size, c * cell_size), thickness)
...
def drawBottomText():
    ...
    #Space
    static_num_dsp = num_font.render(
        "Space: start solve (press again to skip)", True, (0, 0, 0))
    screen.blit(static_num_dsp, (0, 550))

    #GHOST
    static_num_dsp = num_font.render("Shift + num: ghost entry", True,
                                     (0, 0, 0))
...
def gameOverLoop():
    ...
    if not np.any([a == 0 for row in board for a in row]):
        ...
        while GAMEOVERSCREEN:
            ...
            if SIMULATED:
                static_num_dsp = num_font.render("COMPUTER WINS!", True,
                                                 (0, 0, 0))
                screen.blit(static_num_dsp, (105, 550))
            else:
                static_num_dsp = num_font.render(
                    "YOU WIN!    " + str(passed_time / 1000) + "s", True,
                    (0, 0, 0))
...
while RUNNING:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button in (1, 3):
            ...
            highlightcell = (int(mouse_pos[0] / cell_size),
                             int(mouse_pos[1] / cell_size))
        ...
        if event.type == pygame.KEYDOWN:
            ...
            #input ghost entry
            if (event.key in numbers or event.key == pygame.K_BACKSPACE
